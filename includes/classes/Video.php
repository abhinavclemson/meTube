...
class Video
{
    ...
    public function favourites()
    {
        ...
        if ($this->isFavourite()) {
            // User has already made it favourite
            $query = $this->con->prepare("DELETE FROM favourites WHERE username=:username AND videoId=:videoId");
            $query->bindParam(":username", $username);
            $query->bindParam(":videoId", $id);
            $query->execute();
            ...
            $query = $this->con->prepare("INSERT INTO favourites(username, videoId) VALUES(:username, :videoId)");
            $query->bindParam(":username", $username);
            $query->bindParam(":videoId", $id);
            $query->execute();
    ...
    {
        $query = $this->con->prepare("SELECT * FROM favourites WHERE username=:username AND videoId=:videoId");
        $query->bindParam(":username", $username);
        $query->bindParam(":videoId", $id);
